...
def modify_tfbs_and_calculate_score(sequences, tfbs_positions, pwm_plus, pwm_minus, promoter_regions):
    ...
    return modified_sequences, binding_scores
    ...
    for real_index, real in meme_results.iterrows():
        ...
        for predicted_index, predicted in df_promoters.iterrows():
            ...
            if (predicted['start'] <= real['end']) and (predicted['end'] >= real['start']):
                ...
                strand = real['strand']
                #matching_regions = find_matching_regions(predicted_seq, real_seq, strand)
                ...
                #print('real start',real['start'], real['end'],strand)
                ...
                if matching_regions:
                    ...
                    # 处理 matching_regions 以包括 strand 信息
                    
                    tfbs_positions = [(region[0], region[1], strand) for region in matching_regions]

                    # 获取 -35 和 -10 区域信息
#                     top_result = scan_sequence_for_regions_and_create_dataframe(predicted_seq, pwm_35, pwm_10)
                    ...
                    # 计算并获取修改后的序列和得分
                    modified_seqs, scores = modify_tfbs_and_calculate_score(
                        [predicted_seq],
                        tfbs_positions,
...
#                     print(matching_regions)
                    ...
                    print(f"Matched Promoter {predicted_index}: Start = {promoter_start}, End = {promoter_end}")

                    print("Modified Sequence with Scores:")
                    print(marked_seq)
                    print("Binding Scores:",scores[0])
                    promoter_count += 1  # 对每个找到的启动子递增计数器
                    promoter_header = f"{'promoter' + str(promoter_count) + ' ' + '*' * 50}\nMatched Promoter {predicted_index}: Start = {promoter_start}, End = {promoter_end}"
